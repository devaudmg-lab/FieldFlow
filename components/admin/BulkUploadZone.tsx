...
export default function BulkUploadZone({ jobCategory }: BulkUploadProps) {
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // 1. Pehle JSON data ko extract karein
        const result = await response.json();

        if (!response.ok) {
          // 2. Error message ko result se nikalien
          throw new Error(result.error || "Failed to upload data");
        }

        setStatus("success");

        // 3. Backend se aaye "message" ka use karein
        alert(`${result.message} Category: ${jobCategory}`);
      } catch (err: any) {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
      ...
      <label
        className={`flex flex-col items-center justify-center w-full h-52 border-2 border-dashed rounded-xl cursor-pointer transition
          ...
          {loading ? (
            ...
            <>
              ...
              <p className="text-sm text-slate-600 font-medium">
                Click to upload{" "}
                <span className="text-blue-600 font-bold">{jobCategory}</span>{" "}
                Excel
              </p>
      ...
      <div className="mt-4 p-3 bg-slate-50 rounded-lg border border-slate-100">
        <div className="flex items-start gap-2">
          ...
          <p className="text-[11px] text-slate-600 leading-relaxed">
            Names in the Excel will be automatically matched against{" "}
            <b>Profile Aliases</b>. Ensure columns match the template for{" "}
            {jobCategory} projects.
          </p>
        </div>
      </div>
    </div>
